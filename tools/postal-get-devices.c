      ...
      gchar *argv[])
{
   ...
   context = g_option_context_new(_("- Fetch users devices from Postal"));
   ...
   uri_string = g_strdup_printf(
         "http://%s:%d/v1/users/%s/devices",
         (gHost && *gHost) ? gHost : "localhost",
         gPort ? gPort : 5300,
         gUser);
   message = soup_message_new("GET", uri_string);
   g_free(uri_string);
